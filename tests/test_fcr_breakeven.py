...
def _terms(objective, **deltas):
    keys = ("obj_energy_cashflow", "obj_fee_cost", "obj_cycling_cost",
            "obj_imb_cashflow", "obj_imb_vol_penalty", "obj_term_penalty",
            "obj_e_slack_penalty", "obj_reserve_penalty", "obj_balance_penalty")
...
def test_committed_bid_has_positive_margin():
    ...
    m = _fcr_breakeven_metrics(with_bid, without_bid, bid_kw=1000.0, rev_eur=rev,
                               fcr_price=30.0, covered_fraction=1.0)
...
def test_declined_bid_has_non_positive_margin():
    ...
    m = _fcr_breakeven_metrics(forced_in, actual, bid_kw=1000.0, rev_eur=rev,
                               fcr_price=30.0, covered_fraction=1.0)
