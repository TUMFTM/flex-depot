"""Paper plot for the power sensitivity batch: annual gross profit per BET
vs. per-vehicle charging power, one line per fleet.

Run configs are named f<fleet>_p<kW> (e.g. f1_p150); all runs use the full
DA+ID+FCR setup.
"""
...
RUN_NAME_RE = re.compile(r"f(\d+)_p(\d+(?:\.\d+)?)")
...
def _parse_run_name(row: pd.Series) -> tuple[str, float]:
    ...
    raise ValueError(f"Could not parse fleet/power from manifest row: {row.to_dict()}")
...
def load_plot_data(manifest: Path, metric: str) -> pd.DataFrame:
    ...
    duplicate_mask = plot_df.duplicated(["fleet", "power_kw"], keep=False)
    if duplicate_mask.any():
        duplicates = plot_df.loc[duplicate_mask, ["fleet", "power_kw"]]
        raise ValueError(f"Duplicate fleet/power combinations in manifest:\n{duplicates}")

    return plot_df.sort_values(["fleet", "power_kw"]).reset_index(drop=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create the paper-ready power sensitivity figure from a batch manifest."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-base",
        type=Path,
        default=Path("paper/figures/power_sensitivity/power_sensitivity"),
