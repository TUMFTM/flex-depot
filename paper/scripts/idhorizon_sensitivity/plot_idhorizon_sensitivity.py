"""Paper plot for the ID horizon sensitivity batch: annual gross profit per BET
vs. intraday trading horizon, one line per fleet.

Run configs are named f<fleet>_h<hours> (e.g. f1_h04); all runs use the full
DA+ID+FCR setup.
"""
...
RUN_NAME_RE = re.compile(r"f(\d+)_h(\d+)")
...
def _parse_run_name(row: pd.Series) -> tuple[str, int]:
    ...
    raise ValueError(f"Could not parse fleet/horizon from manifest row: {row.to_dict()}")
...
def load_plot_data(manifest: Path, metric: str) -> pd.DataFrame:
    ...
    parsed.columns = ["fleet", "horizon_h"]
    ...
    duplicate_mask = plot_df.duplicated(["fleet", "horizon_h"], keep=False)
    if duplicate_mask.any():
        duplicates = plot_df.loc[duplicate_mask, ["fleet", "horizon_h"]]
        raise ValueError(f"Duplicate fleet/horizon combinations in manifest:\n{duplicates}")
...
def make_figure(plot_df: pd.DataFrame, metric: str, output_base: Path, xscale: str = "log") -> Path:
    ...
    # The horizons span 1-48 h with dense sampling at the short end, so a
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create the paper-ready ID horizon sensitivity figure from a batch manifest."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-base",
        type=Path,
        default=Path("paper/figures/idhorizon_sensitivity/idhorizon_sensitivity"),
