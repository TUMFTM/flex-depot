"""Paper plot for the ageing sensitivity batch: annual gross profit per BET
vs. cycle regularization cost, one line per fleet.

Run configs are named f<fleet>_c<idx> (e.g. f1_c05), varying the cycle
regularization cost (cost_eur_per_kwh_throughput, idx = cost in ct/kWh); all
runs use the full DA+ID+FCR setup.
"""
...
RUN_NAME_RE = re.compile(r"f(\d+)_c(\d+)")


def _parse_run_name(row: pd.Series) -> tuple[str, int]:
    candidates = [
    ...
    for name in candidates:
        ...
        if match:
            return f"F{int(match.group(1))}", int(match.group(2))
    raise ValueError(f"Could not parse fleet/cost index from manifest row: {row.to_dict()}")


def _cycle_cost(row: pd.Series) -> float:
    """Cycle regularization cost in EUR/kWh, read from the batch config
    tracked in the repo; falls back to the c<idx> = ct/kWh naming convention
    ...
    if config_path.is_file():
        ...
        try:
            return float(
                config["optimization"]["flexibility"]["cycle_regularization"][
                    "cost_eur_per_kwh_throughput"
                ]
            )
        except KeyError:
            pass
    return row["cost_idx"] / 100.0
...
def load_plot_data(manifest: Path, metric: str) -> pd.DataFrame:
    ...
    parsed.columns = ["fleet", "cost_idx"]
    plot_df = pd.concat([parsed, df[["config", metric]].astype({metric: float})], axis=1)
    plot_df["cost_eur_per_kwh"] = plot_df.apply(_cycle_cost, axis=1)
    plot_df = plot_df.drop(columns=["config", "cost_idx"])
    ...
    duplicate_mask = plot_df.duplicated(["fleet", "cost_eur_per_kwh"], keep=False)
    if duplicate_mask.any():
        duplicates = plot_df.loc[duplicate_mask, ["fleet", "cost_eur_per_kwh"]]
        raise ValueError(f"Duplicate fleet/cost combinations in manifest:\n{duplicates}")

    return plot_df.sort_values(["fleet", "cost_eur_per_kwh"]).reset_index(drop=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create the paper-ready ageing sensitivity figure from a batch manifest."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
